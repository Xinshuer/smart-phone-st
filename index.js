...
function onPromptReady(eventData) {
    ...
    // v0.14.64 补 "看 + 身体部位" 类（"看看小穴"/"看下奶"/"瞧瞧屁股" 等 user 直接索取身体部位的句式）
    // 之前只覆盖了"看看你/她/妈/姐"代词形式，没覆盖具体部位 → 用户说"看看小穴"被判 isImageRequest=false
    // → noPicMetaRule 注入"严禁嵌 pic" → AI 只发文字不发图。补全后命中这类索取
    const BODY_PART = '(?:小穴|阴蒂|阴唇|阴道|阴部|蜜穴|逼|阴|穴|奶子|奶头|乳头|乳房|乳|胸|屁股|后庭|肛门|菊花|腿|大腿|脚|脸|脖|肩|腰|背|腰窝|身体|身材|大腿根|腹|肚脐|私处|下面|那里|宝贝|内裤|底裤|内衣|胸罩|罩杯|腋下|锁骨|脖颈|乳沟|腰肢|尾骨|臀沟|臀缝)';
    const IMAGE_REQUEST_RE = new RegExp(
        '(?:'
        + '照片|相片|图片|图像|拍照|拍张|拍个|拍组|拍.{0,5}张|拍.{0,5}相'
        + '|拍腿|拍奶|拍胸|拍屁股|拍小穴|拍逼|拍阴|拍脚|拍脸|拍全身|拍背|拍脖|拍肩|拍腰|拍上半身|拍下半身'
        + '|发图|发照|发张|发一张|发几张|发个图|发一组|发自拍|发个|发组'
        + '|看看你|看看她|看看妈|看看姐|看看你的|看看她的|看看妈的|看看姐的'
        + '|看(?:看|下|到|一下|一眼|两眼|两下)?' + BODY_PART       // "看小穴"/"看看小穴"/"看下奶"/"看一眼屁股"
        + '|想看' + BODY_PART + '|让我看|让.{1,3}看|让大家看|让他们看|让他看|让她看|给我看|给老子看'
        + '|瞧(?:瞧|一瞧|一下)?' + BODY_PART
        + '|秀(?:一下|一秀|秀)?' + BODY_PART
        + '|露(?:一下|出|一露)?' + BODY_PART
        + '|展示' + BODY_PART
        + '|自拍|穿搭|镜子前|视频|录像|直播|走光'
        + '|露(?:点|奶|逼|穴|胸|屁股)'
        + '|脱.{0,3}拍|脱了发'
        + '|show me|selfie'
        + ')',
