...
export async function generateFreshFeed(chatId, ctx) {
    const charName = ctx?.name2 || ctx?.name || 'TA';
    const userName = ctx?.name1 || '用户';

    const sysMsg = `你是一个小红书内容生成器。生成3条**陌生人/路人**视角的随机帖子。
每条要求：
- title 标题 8-15字
...
**只返回 JSON 数组**：[{"user":"作者emoji昵称","tag":"标签","title":"标题","body":"正文","likes":数字,"comments":[{"user":"昵称","text":"评论"}]}]`;
