    ...
    return `
        <div class="phone-settings">
            ...
            <section class="phone-settings-section">
                ...
                <label class="phone-settings-row">
                    模型名称
                    <input id="phone-api-model" type="text" class="phone-input" list="phone-api-model-list" value="${escapeHtml(s.api?.model || '')}" placeholder="deepseek-chat">
                    <datalist id="phone-api-model-list">
                        ${MODEL_SUGGESTIONS.map((m) => `<option value="${escapeHtml(m)}">`).join('')}
                    </datalist>
...
export function bindSettingsHandlers(root, { onImportContact, onGenRef, onLockRef, onRemoveContact, onModelChange, onComfyuiUrlChange, onToggleLore, onRefresh, onApiSave, onApiTest, onApiTriggerNow, onComfyuiTest, onFetchModels, onPromptEdit, onGenAppearance }) {
    ...
    const modelSel = root.querySelector('#phone-model-select');
    if (modelSel) modelSel.addEventListener('change', () => onModelChange(modelSel.value));
    ...
    const modelInput = root.querySelector('#phone-api-model');
    ...
    [urlInput2, keyInput, modelInput, triggerSel].forEach((el) => {
    ...
    root.querySelector('#phone-api-fetch-models')?.addEventListener('click', async () => {
        ...
        if (list && list.length) {
            const dl = root.querySelector('#phone-api-model-list');
