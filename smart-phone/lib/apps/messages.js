...
export function renderThread(chatId, threadName) {
    ...
    const avatarHtml = refImg
        ? `<img class="phone-avatar phone-avatar-ref small" src="${escapeHtml(refImg)}" alt="${escapeHtml(threadName.slice(0, 1))}" onerror="this.outerHTML='<div class=\\'phone-avatar small\\'>${escapeHtml(threadName.slice(0, 1))}</div>'">`
        : `<div class="phone-avatar small">${escapeHtml(threadName.slice(0, 1))}</div>`;
...
function renderBubble(m) {
    const cls = m.me ? 'phone-bubble me' : 'phone-bubble';
...
function renderContent(m) {
    switch (m.type) {
        ...
        case 'image':
            return m.pic
                ? `<div class="phone-image-slot" data-pic="${escapeHtml(m.pic)}">📷 生成中…</div>`
                : `<div class="phone-image-slot">${escapeHtml(m.content)}</div>`;
        default:
            return `${escapeHtml(m.content)}${
                m.pic ? `<div class="phone-image-slot" data-pic="${escapeHtml(m.pic)}">📷 生成中…</div>` : ''
            }`;
    }
}
