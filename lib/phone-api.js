...
export async function callPhoneApi(userPrompt, sysMsg = '', opts = {}) {
    ...
    // v0.14.26 same internal-call flag as callViaMainPreset — generateRaw also fires
    // CHAT_COMPLETION_PROMPT_READY which would inject our PHONE protocol if not skipped.
    try {
...
async function callViaMainPreset(userPrompt, sysMsg = '') {
    try {
        ...
        // v0.14.26 set global flag so onPromptReady SKIPS PHONE protocol injection +
        // skip preset-token strip. Otherwise NPC comment requests get the PHONE protocol
        // pushed in and AI outputs <PHONE> blocks instead of the JSON array we need.
        window.__smartPhoneInternalQuietCall = true;
        try {
