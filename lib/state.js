...
export const defaults = {
    ...
    imageGen: {
        ...
        // v0.14.49 双 Pass 出图加速：
        //   Pass 1 (主 AI) 用 <pic/> 占位符替代完整 <pic prompt="..."/>，输出 token 砍半 → 主消息更快
        //   Pass 2 (plugin 后台 generateQuietPrompt) 给每个占位符单独生成 booru prompt
        //   预期：0 图聊天 ~65% 加速，含图聊天 ~40-55% 加速
        splitPicGen: false,
    },
};
...
// v0.14.48 fix：用 saveSettings（immediate）替代 saveSettingsDebounced，防 user 在
// debounce 窗口内刷新 → 保存没 fire → 重新出图。pic URL 关键持久化数据不能等 debounce。
...
// v0.14.78 把 ComfyUI 返回的 URL 写回 feed post，让下次 render 直接 <img>，
// 跳过 slot+triggerPicSlots 全套（修"退出帖子再进同一个帖子生成新图"bug）。
// idx=0 = 单图覆盖 post.pic / 多图替换对应 index。
// resolvedFromPic 标记区分"自动写回"vs"user 上传"，reroll 时只清自动写回的。
export function attachFeedPostImage(chatId, platform, postId, idx, url) {
    if (!platform || !postId || !url) return;
    const cs = getChatState(chatId);
    let arr;
    if (platform === 'xhs') arr = cs.xhs;
    else if (platform === 'moments') arr = cs.moments;
    else if (platform === 'forum') arr = cs.forum;
    if (!Array.isArray(arr)) return;
    const post = arr.find((p) => p.id === postId);
    if (!post) return;
    // 如果 post 已经是 user 上传 / 命令附图（多张 URL），不动；只在没 images 或自动写回模式下设置
    if (Array.isArray(post.images) && post.images.length > 0 && !post.resolvedFromPic) return;
    if (!Array.isArray(post.images)) post.images = [];
    post.images[idx] = url;
    post.resolvedFromPic = true;
    saveSettingsDebounced();
}

// 配套 reroll：picTag 对应的"自动写回 post.images"清掉，让下次 render 走 slot 重生成
export function detachFeedPostImageByPic(chatId, picTag) {
    if (!picTag) return;
    const cs = getChatState(chatId);
    let changed = false;
    for (const platform of ['xhs', 'moments', 'forum']) {
        const arr = cs[platform];
        if (!Array.isArray(arr)) continue;
        for (const post of arr) {
            if (post.resolvedFromPic && post.pic === picTag) {
                delete post.images;
                delete post.resolvedFromPic;
                changed = true;
            }
        }
    }
