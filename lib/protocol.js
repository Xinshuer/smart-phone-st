...
如果 user 在请求里**明确指定了姿势或身体部位**，pic prompt **必须严格还原**，不能自由发挥成其他姿势：

| user 表达 | pic prompt 必须含 |
|---|---|
| "拍腿 / 看腿 / 秀腿 / 美腿" | \`legs focus, thigh focus, (long legs:1.2)\`，适当加 \`from below\` |
| "拍胸 / 看胸 / 秀奶 / 露奶" | \`breast focus, cleavage focus, large breasts\`，适当加 \`cowboy shot\` |
| "拍屁股 / 翘屁股 / 撅屁股" | \`(ass focus:1.3), huge ass, plump ass, from behind\` 或 \`top-down bottom-up\` |
| "拍小穴 / 看小穴 / 给我看穴" | \`(pussy focus:1.3), spread legs, spread pussy, close-up, pov\` |
...
**通用质量约束**（始终遵守）：
- 角色衣物和身体不能糊成一体——确保 pic prompt 含 \`detailed clothes, clear silhouette\`
- NSFW 时优先用 \`(detailed pussy:1.3)\` 等加权 tag 提升解剖准确度
...
export function buildSendOOC({ targetName, time, userText = '', isGroup = false, groupName = '', memberNames = [] }) {
    ...
    if (isGroup) {
        ...
        return `[模式 A 触发——手机群聊]\n群名：${groupName}\n时间：${time}\n${userLine}\n\n按系统提示中的手机协议输出 PHONE 块（包含群消息标签 GMSG），块外一字不写（不写 prose、不写旁白）。GROUP 属性用 "${groupName}"，TIME 属性用 "${time}"。${allowed}${imageHint}${bodyPartHint}`;
    }

    return `[模式 A 触发——手机短信]\n目标角色：${targetName}\n时间：${time}\n${userLine}\n\n**强制规则**：\n- 按系统提示中的手机协议输出 PHONE 块，块外一字不写（不写 prose、不写旁白）\n- 至少一条 SMS 标签（FROM 属性="${targetName}"，TIME 属性="${time}"，标签里写回复内容），建议 2-4 条连发\n- FROM 严格用 "${targetName}"，禁翻译/别名\n- 是 ${targetName} 自己的话，不要复述用户内容${imageHint}${bodyPartHint}`;
}
