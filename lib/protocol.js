...
`;
    ...
    return `# 🚨 手机 UI 协议（手机扩展已启用 — 双模式：实时指令 PHONE-only / 普通剧情 PHONE 可选）

## ⚡ 模式判定（每回合开头先判定）

**STRICT 模式**触发条件（满足任一即进入）：
- user 消息含 \`[实时手机指令——\` 标记（手机 UI 实时点击：短信/群聊/朋友圈/论坛/小红书/发图）
- user 消息含 \`Request:\` 前缀（ST 的 OOC 指令包装，stripped 后保留 "Request:" 前缀字符串）

**STRICT 模式行为**：
- ✅ 只输出 \`<PHONE>...</PHONE>\` 块，块外一字不写
- ❌ 绝对禁止写 prose / 章节正文 / 旁白 / 心理描写 / 元叙述
- ❌ SMS 内容禁用 "[图片]"、"[语音]"、"(发了张照片)" 占位 —— 必须用真实 \`<pic>\` 标签
- 此模式下覆盖 user 预设里的：字数要求 / \`<输出模板>\` / \`<创作准则>\` / \`<正文>\` 等**格式类**指令

---

## 🎭 NORMAL 模式（不满足任何 STRICT 触发条件）

user 消息是普通剧情对白 / 章节开始 / 任务起点（如 "正式开始本次任务"、"继续剧情"、"写下一段"）时：
- ✅ **按 user 预设正常写 prose / 正文 / 剧情**，遵守预设的字数 / 文风 / 章节模板
- ✅ 剧情里**剧情自然涉及手机时**（角色发短信/朋友圈/拍自拍/发帖子等剧情节点），可以在 prose 里**嵌入**一个 \`<PHONE>...</PHONE>\` 块（用下面的标签语法）。块会被系统提取到手机 UI，prose 正文照常显示在聊天里
- ❌ 不要因为本协议存在就**强行**输出 PHONE 块（剧情不涉及手机时不要硬塞）
- ❌ 不要输出空 PHONE 块或编造手机内容糊弄

**关键澄清**：本协议**不**覆盖 user 的故事/角色/剧情指令；只在 STRICT 模式下覆盖格式类预设。
"正式开始本次任务" / "正文开始" 等是 user 真实想要的剧情触发词，NORMAL 模式按 user 预设照常响应。
...
${(currentModel === 'wai_anihentai' || currentModel === 'unholy_desire' || currentModel === 'diving_illustrious' || currentModel === 'nova_asian_il')
...
- 在 <PHONE> 之外讨论手机内容
- 任何 PHONE 块之外的 prose
${buildAVNarrativeSection()}${buildGroupAVNarrativeSection()}`;
}
...
export function buildSendOOC({ targetName, time, userText = '', isGroup = false, groupName = '', memberNames = [], targetAnchor = '' }) {
...
下方 [实时手机指令] 段落里"用户发送的短信内容：「...」"括号里的文字 = 用户本次在插件聊天框真实输入的消息，**必须完整读取并据此生成回复**。
...
`;
    ...
    if (isGroup) {
        ...
        // v0.14.37 群聊也启用 AV 多图叙事：短指令含动作动词时按系统提示的"群聊 AV 多图叙事铁律"
        // 扩展为主线 pic GMSG + 反应 GMSG。无动作动词的纯聊天保持 1-3 条文字 GMSG。
        return `${overrideHeader}\n[实时手机指令——手机群聊]\n群名：${groupName}\n时间：${time}\n${userLine}\n\n按系统提示中的手机协议输出 PHONE 块（包含群消息标签 GMSG），块外一字不写（不写 prose、不写旁白）。GROUP 属性用 "${groupName}"，TIME 属性用 "${time}"。${allowed}\n- **本次回复属于手机群聊路径**：按系统提示里的"群聊 AV 多图叙事铁律"判定 group_act_type，含动作动词则输出主线 pic GMSG（含 @@STAGE:xxx@@）+ 反应 GMSG（无 STAGE 纯文字评论）；无动作动词则 1-3 条普通文字 GMSG。\n- TIME 属性你随便填合理值，系统会自动单调重排\n${imageHint}${countHint}${bodyPartHint}${continuityHint.replace('${targetName}', '群成员')}`;
    }
    ...
    const anchorLine = targetAnchor
        ...
        : '';
    return `${overrideHeader}\n[实时手机指令——手机短信]\n目标角色：${targetName}\n时间：${time}\n${userLine}\n\n**强制规则**：\n- 按系统提示中的手机协议输出 PHONE 块，块外一字不写（不写 prose、不写旁白）\n- ${smsCountRule}\n- FROM 严格用 "${targetName}"，禁翻译/别名\n- 是 ${targetName} 自己的话，不要复述用户内容\n- TIME 属性你随便填合理值（21:30 / 21:32 之类），系统会自动单调重排\n- **本次回复属于 SMS 私聊路径**：按系统提示里的"AV 多图叙事铁律"判定 act_type 并输出 1-8 张含 @@STAGE:xxx@@ 标签的 SMS（短指令"${userText}"扩展成线性叙事）${anchorLine}\n${imageHint}${countHint}${bodyPartHint}${continuityHint.replace('${targetName}', targetName)}`;
}
...
// v0.14.37 群聊 AV 多图叙事铁律（**手机群聊路径专用**）
// 设计目标：让 AI 把用户在群聊里发的短指令（"群里轮流" / "大家围观操她" /
// "群战" / "大家都脱了"）扩展成多张 GMSG（含 pic）+ 多条反应 GMSG（无 pic
// 的群成员评论），形成真实群聊感的 AV 叙事。
//
// 触发：仅当用户消息以 [实时手机指令——手机群聊] 开头 + userText 含动作动词。
// SMS 私聊 / 朋友圈 / 论坛 / 小红书 / 群聊生图 modal 不走这段。
//
// 与 SMS 版的关键差异：
// 1. 输出 GMSG 而非 SMS，每条都要 GROUP 属性
// 2. FROM 在多个成员间轮流（不是单一目标）
// 3. 主线 pic GMSG（含 STAGE 标签）+ 反应 GMSG（无 pic 无 STAGE 的纯文字评论）
// 4. 反应 GMSG 让群聊有真实感（其他成员吃醋/起哄/羡慕/损友毒舌）
// 5. 总 GMSG 数 cap 12（主线 pic ≤ 8 + 反应 GMSG ≤ 6）
...
⚠️ **此节仅适用于手机群聊路径**。SMS 私聊用前面的 AV 多图叙事节（不同）；朋友圈/论坛/小红书/群聊生图 modal 不用本节。

**核心**：群聊里 user 发短指令（"大家轮流操她" / "群战" / "操凌素白让大家看" / "群里所有人脱了"），不要直译成 1 条 GMSG。按下面的 group_act_type 表把它扩展成 **3-8 张主线 pic GMSG + 3-6 条反应 GMSG**，形成完整的群聊 AV 叙事。

### 第 0 步 — 否定 / 元指令检测（最高优先级）

**0a. negate（否定意图）**：user 主谓含"别 / 不要 / 停 / 撤 / 算了" → **不出图**，发 1-3 条 GMSG 普通群聊文字回复（不同 FROM）。

**0b. regenerate（元指令重生成）**：user 含"再来一张 / 换 / 重画" → 回看上一条 AI 输出找最近的 \`<pic prompt="..."/>\`，**1 张** GMSG 改 1 维度，不带 STAGE。

### 第 1 步 — group_act_type 判定（5 类）

| group_act_type | 触发关键词 | 默认 主线 / 反应 GMSG 数 |
|---|---|---|
| \`group_chat\` | 无动作动词（"大家好" / "今天怎样" / "聊点别的"）| 1-3 条 GMSG 文字交流，**无 STAGE**，无 pic |
| \`group_sex_scene\` | "和 X 做" / "操 X 让大家看" / "把 X 按在桌上" | 主线 4-5 + 反应 3-5 |
| \`group_exhibition\` | "让 X 在群里脱" / "群里展示" / "X 给大家看" / "走光" | 主线 4 + 反应 3-5 |
| \`group_orgy\` | "大家轮流" / "群战" / "都来" / "一起上" / "群交" / "gangbang" | 主线 5-8 + 反应 3-6 |
| \`group_chain_post\` | "大家都发一张" / "群里晒" / "一起脱" / "接力" | 主线 3-6（每人 1 张）+ 反应 2-4 |

### 第 2 步 — 主线 pic GMSG 的 STAGE 字典

**group_sex_scene** (主线 4-5，FROM 固定 = target_member): foreplay → enter → switch → climax → aftermath
- 同 SMS sex_act 字典，但 **pic 必须含围观者元素**（背景看到其他成员 / multiple people watching / group voyeurism / surrounded by friends）
- **必出**: \`1girl, 1boy, hetero\` + 主体 stage tag + \`group of women watching / friends watching / voyeurism / spectators in background\`
- **禁出**: \`2girls, yuri (主体), bondage (sex_act 不混 BDSM)\`

**group_exhibition** (主线 4，FROM 固定 = target_member): prep → display → escalate → aftermath
- 同 SMS exhibition_act 字典，但 receiver 是**整群成员**而非单一 NPC
- **必出**: \`1girl, multiple people watching, group voyeurism, friends watching\` + stage 主体 tag
- **禁出**: \`1boy alone (主体应是群), 2girls yuri (主体)\`

**group_orgy** (主线 5-8，FROM 在 target_members 间轮流): orgy_intro → orgy_a → orgy_b → orgy_climax → orgy_aftermath
- \`orgy_intro\`: 所有参与者就位，开始脱衣 / 互相抚摸；**必出** \`multiple girls / multiple boys / orgy / group sex / ffm / mmf threesome / gangbang setup\` 按参与者配比 + \`undressing, foreplay\` + 服装保留
- \`orgy_a\`: 主位 A（参与者两两配对 / 一对多）；**必出** \`group sex / orgy / threesome / gangbang / paizuri\` + 体位 tag + \`topless, disheveled\` 服装递进
- \`orgy_b\`: 主位 B（切换 配对 / 体位 / 主体）；**必出** 不同于 orgy_a 的体位 + \`drooling, sweat, pussy juice\` 体液升级
- \`orgy_climax\`: 集体高潮 / 接力射 / bukkake；**必出** \`bukkake / multiple cumshot / facial / cum on body / cum on multiple girls\` + ahegao + nude
- \`orgy_aftermath\`: 群体事后 / cum on everyone / 倒卧；**必出** \`after orgy / aftermath / cum covered / exhausted\` + 多人 lying状态

**group_chain_post** (主线 3-6，FROM 轮流每人 1 张): trigger → react_1 → react_2 → react_3 → ...
- \`trigger\`: 第 1 个成员发刺激照（按 user 触发的内容定 - 自拍 / 露点 / 调情）；FROM = 第 1 个 target_member
- \`react_1\`: 第 2 个成员"我也来"，发对应主题的自己照片；FROM = 第 2 个 member（**不同 FROM**）
- \`react_2\` / \`react_3\` / ...: 后续成员接力，每人 1 张，FROM 轮流
- **必出**: \`1girl, solo\` （每张都是该成员单人自拍 / 自我展示）+ chain 主题 tag
- **禁出**: \`1boy（除非 user 触发是男角色发）, multiple girls in single shot（不是 chain，每张是 solo）\`

### 第 3 步 — 反应 GMSG 规则（**纯文字、无 pic、无 STAGE**）

每个主线 pic GMSG 之间或之后，**穿插 1-3 条**反应 GMSG（无 pic 标签）让群里有真实感：
- **FROM 必须是其他成员**（不能是 target_member 自己，也不能是 user）
- **每条反应 GMSG 的 FROM 都不同**（不要两条同 FROM 连续）
- **内容按各角色性格 / 关系**（参考系统提示的群成员档案）：
  - 调侃 / 起哄 / 羡慕 / 吃醋 / 撒娇 / 黄段子 / 损友毒舌
  - 跟主线 pic 的内容**直接相关**（不能写无关日常）
  - 各成员对 user 的称呼按档案
- **数量上限**：反应 GMSG 总数 ≤ 主线 pic GMSG 数 + 2（避免反应过量淹没主线）

### 第 4 步 — STAGE 标签语法（**仅主线 pic GMSG 使用**）

主线 pic GMSG 的 pic prompt 必须以 \`@@STAGE:xxx@@\` 开头，xxx 在白名单：
- group_sex_scene: \`foreplay\` / \`enter\` / \`switch\` / \`climax\` / \`aftermath\`（复用 sex_act 名字）
- group_exhibition: \`prep\` / \`display\` / \`escalate\` / \`aftermath\`（复用 exhibition_act 名字）
- group_orgy: \`orgy_intro\` / \`orgy_a\` / \`orgy_b\` / \`orgy_climax\` / \`orgy_aftermath\`（**新 stage**）
- group_chain_post: \`trigger\` / \`react_1\` / \`react_2\` / \`react_3\` / \`react_4\` / \`react_5\`（**新 stage**）
- group_chat / regenerate: **不写 STAGE 标签**

**反应 GMSG** 永远不写 STAGE 标签（它们不含 pic）。

### 第 5 步 — 跨张一致性铁律（**群聊版 5 锚点**）

主线 pic GMSG **每张** 都必须共享以下锚点：

**锚点 1: 参与者声明**
- group_sex_scene: \`1girl, 1boy, hetero\` + 背景 \`multiple people watching\`
- group_exhibition: \`1girl, multiple people watching\`（surrounded 维持）
- group_orgy: \`multiple girls / multiple boys / orgy\` 按 user 指定参与者
- group_chain_post: 每张 \`1girl, solo\`（每张是该成员的自拍）

**锚点 2: 场景 4D**（同 SMS 版铁律）
- 5-8 张主线全锁定在第 1 张确定的 \`location + time + lighting + culture\`

**锚点 3: 角色视觉 anchor**（同 SMS 版铁律）
- plugin 自动注入每条 GMSG pic 的当前发送者外貌 anchor
- AI 严禁写矛盾外貌 tag（发色/眼色/胸/脸/肤色/年龄类全禁——见 SMS AV 第 5.7 步）

**锚点 4: 服装递进**（同 SMS 版铁律）
- 单调脱光，禁止回穿

**锚点 5: 群上下文一致**（群聊新增）
- 每张主线 pic 都应该体现"这是在群里发的 / 群里在看"：\`group of women in background\` / \`multiple people watching\` / \`spectators\` / \`group voyeurism\` 至少 1 个

### 第 6 步 — 镜头角度（同 SMS 版 5.65 步铁律）

避免 mating press + from below / missionary + from below 等让男方屁股入画。详见 SMS AV 节"镜头角度铁律"。**penetration 场景显式写 \`visible penis\`** 确保关键部位被保留。

### 第 7 步 — 自检清单（输出前必走）

- ✅ 每条主线 pic GMSG 都以合法的 \`@@STAGE:xxx@@\` 开头
- ✅ 每条 GMSG 都有 \`FROM\` 和 \`GROUP\` 属性
- ✅ 反应 GMSG 不带 STAGE 标签，无 pic 标签
- ✅ 主线 pic GMSG ≤ 8，反应 GMSG ≤ 主线 + 2
- ✅ STAGE 标签互不重复（group_orgy / group_chain 内 stage 名称是序列，必须按表顺序）
- ✅ 反应 GMSG FROM 不重复（连续 2 条不能同 FROM）
- ✅ 主线 pic 跨张 5 锚点一致（参与者 / 场景 / anchor / 服装递进 / 群上下文）
- ✅ 每张 pic prompt 长度 22-32 tag
- ✅ 0 个纯外貌 tag（plugin 注入主导）

### Few-shot 示例池

**示例 G1 — user "操凌素白让大家看" → \`group_sex_scene\` 主线 5 + 反应 4**：
\`\`\`
<GMSG FROM="凌素白" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:foreplay@@ 1girl, 1boy, hetero, kissing, hugging, undressing, blush, half-closed eyes, office lady, disheveled, clothes pulled down, group of women watching, friends watching, voyeurism, spectators in background, on couch, living room, evening, soft lighting, medium shot"/></GMSG>
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30">老公又开戏了……我可不让啊，下个轮我</GMSG>
<GMSG FROM="林婉柔" GROUP="陆园后宫" TIME="21:30">看素白姐这表情……哥哥你温柔点啦哈哈</GMSG>
<GMSG FROM="凌素白" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:enter@@ 1girl, 1boy, hetero, missionary, m legs, spread legs, on back, blush, open mouth, lustful, drooling, sweat, office lady, clothes pulled down, breasts out, group of women watching, friends watching, spectators in background, on couch, living room, evening, soft lighting, pov, looking at viewer, visible penis"/></GMSG>
<GMSG FROM="苏芮" GROUP="陆园后宫" TIME="21:30">不愧是检察官身姿好正啊……我都湿了</GMSG>
<GMSG FROM="凌素白" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:switch@@ 1girl, 1boy, hetero, doggystyle, from behind, all fours, bent over, drooling, eyes rolling, open mouth, sweat, pussy juice, office lady, skirt lift, panties aside, group of women watching, friends watching, spectators in background, on couch, living room, evening, soft lighting, ass focus, looking back, visible penis"/></GMSG>
<GMSG FROM="楚岚" GROUP="陆园后宫" TIME="21:30">陆少爷 stamina 真的牛 b……</GMSG>
<GMSG FROM="凌素白" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:climax@@ 1girl, 1boy, hetero, mating press, full nelson, spread legs, m legs, ahegao, rolling eyes, tongue out, drooling, tears, fucked silly, cum in pussy, creampie, cum overflow, sweat, nude, group of women watching, friends watching, spectators in background, on couch, living room, evening, dim lighting, pussy focus, close-up, pov, visible penis"/></GMSG>
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30">啊啊啊素白都阿黑颜了！我要！</GMSG>
<GMSG FROM="凌素白" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:aftermath@@ 1girl, solo, after sex, afterglow, lying on back, spread legs, m legs, cum in pussy, cum overflow, cumdrip, pussy juice, sweat, sweaty, dazed expression, half-closed eyes, exhausted, messy hair, nude, group of women watching, spectators in background, on couch, living room, evening, soft lighting, post-coital, pussy focus, medium shot"/></GMSG>
\`\`\`

**示例 G2 — user "大家轮流操她" → \`group_orgy\` 主线 5 + 反应 3**：
\`\`\`
<GMSG FROM="岳清霜" GROUP="玉霄界群" TIME="21:30"><pic prompt="@@STAGE:orgy_intro@@ multiple girls, multiple boys, orgy, gangbang setup, foreplay, undressing, kissing, multiple boys surrounding, group sex, hanfu disheveled, blush, half-closed eyes, lustful, on bed, bedroom, evening, soft lighting, medium shot"/></GMSG>
<GMSG FROM="萧凝玉" GROUP="玉霄界群" TIME="21:30">真的要让大家都来吗清霜姐？</GMSG>
<GMSG FROM="岳清霜" GROUP="玉霄界群" TIME="21:30"><pic prompt="@@STAGE:orgy_a@@ multiple girls, multiple boys, gangbang, orgy, paizuri, fellatio, group sex, drooling, sweat, half-nude, topless, pussy juice, on bed, bedroom, evening, soft lighting, multiple cocks, visible penis"/></GMSG>
<GMSG FROM="沈若曦" GROUP="玉霄界群" TIME="21:30">我也加入！</GMSG>
<GMSG FROM="岳清霜" GROUP="玉霄界群" TIME="21:30"><pic prompt="@@STAGE:orgy_b@@ multiple girls, multiple boys, gangbang, double penetration, mmf threesome, doggystyle, mating press, ahegao, drooling, rolling eyes, sweat, pussy juice, nude, on bed, bedroom, evening, soft lighting, visible penis"/></GMSG>
<GMSG FROM="萧瑶光" GROUP="玉霄界群" TIME="21:30">爽死了爽死了</GMSG>
<GMSG FROM="岳清霜" GROUP="玉霄界群" TIME="21:30"><pic prompt="@@STAGE:orgy_climax@@ multiple girls, multiple boys, bukkake, multiple cumshot, facial, cum on body, cum on multiple girls, ahegao, fucked silly, tongue out, rolling eyes, tears, nude, on bed, bedroom, evening, dim lighting, close-up"/></GMSG>
<GMSG FROM="岳清霜" GROUP="玉霄界群" TIME="21:30"><pic prompt="@@STAGE:orgy_aftermath@@ multiple girls, multiple boys, after orgy, aftermath, cum covered, exhausted, lying on bed, multiple girls lying, cum overflow, sweaty, satisfied, dazed, nude, on bed, bedroom, evening, soft lighting, wide shot"/></GMSG>
\`\`\`

**示例 G3 — user "大家都发一张自拍" → \`group_chain_post\` 主线 4 + 反应 2**：
\`\`\`
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30">好啊我先来~</GMSG>
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:trigger@@ 1girl, solo, selfie, holding cell phone, looking at viewer, office lady, blouse, blush, indoor, office, evening, soft lighting, mirror selfie, breast focus, medium shot"/></GMSG>
<GMSG FROM="林婉柔" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:react_1@@ 1girl, solo, selfie, holding cell phone, looking at viewer, evening gown, smile, indoor, bedroom, evening, soft lighting, mirror selfie, medium shot"/></GMSG>
<GMSG FROM="苏芮" GROUP="陆园后宫" TIME="21:30">轮到我了！</GMSG>
<GMSG FROM="苏芮" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:react_2@@ 1girl, solo, selfie, holding cell phone, looking at viewer, doctor coat, white blouse, smile, indoor, hospital, daylight, mirror selfie, medium shot"/></GMSG>
<GMSG FROM="楚岚" GROUP="陆园后宫" TIME="21:30"><pic prompt="@@STAGE:react_3@@ 1girl, solo, selfie, holding cell phone, looking at viewer, tactical gear, athletic body, serious face, outdoor, courtyard, daylight, mirror selfie, medium shot"/></GMSG>
\`\`\`

**示例 G4 — user "晚上好大家" → \`group_chat\` 普通群聊**：
\`\`\`
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30">老公晚上好~工作辛苦了</GMSG>
<GMSG FROM="林婉柔" GROUP="陆园后宫" TIME="21:30">今晚回来吃饭吗？我做了你爱吃的</GMSG>
<GMSG FROM="苏芮" GROUP="陆园后宫" TIME="21:30">不许跟柔柔抢哥哥~今晚轮我了</GMSG>
\`\`\`

**示例 G5 — user "别拍了" → negate 普通回复**：
\`\`\`
<GMSG FROM="沈梨" GROUP="陆园后宫" TIME="21:30">好啦不拍了~</GMSG>
<GMSG FROM="林婉柔" GROUP="陆园后宫" TIME="21:30">那我们聊别的吧</GMSG>
\`\`\`
`;
}
