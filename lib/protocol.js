...
`
    ...
    // v0.14.49 双 Pass 加速模式 — 主 AI 用 <pic/> 占位符，pic prompt 由 plugin 后台单独生成
    const placeholderModeRule = splitPicGen
        ? `\n## ⚡ 双 Pass 加速模式（本插件已开启 — 最高优先级）\n\n**仅本回合**：所有 pic 标签**严禁**写 prompt 属性，**只写占位符** \`<pic/>\`。\n\n- ✅ 正确：\`<SMS FROM="X" TIME="14:32">话语 <pic/></SMS>\`\n- ❌ 错误：\`<SMS FROM="X" TIME="14:32">话语 <pic prompt="1girl, ..."/></SMS>\`\n- 原因：plugin 会用后台 quiet AI 单独生成 booru prompt（基于本条 SMS 内容 + 角色 anchor + user 原话），输入更聚焦 → 生成更精准；同时主 AI 输出 token 砍半 → 主消息速度更快\n- ⚠️ 其他属性照写：SUBJECT="X"（A 发 B 照片）/ SUBJECTS="A,B"（合影）— 这些跟 prompt 无关，正常带\n\n下面协议里所有"pic prompt 必须含 XX"的规则**本回合一概不适用**（plugin 后台 AI 会处理）。但**pic 数量/位置/分布**仍按 AV 多图叙事铁律决定。\n\n---\n`
        : '';
