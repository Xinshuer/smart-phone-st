...
export function buildProtocolPrompt({
    ...
    splitPicGen = false,
    // v0.14.62: pre-fire 预生成的图数量 — 协议强制 AI 输出**恰好** N 张 pic，跟预生成 1:1 对位
    prefireCount = 0,
} = {}) {
...
`;
    ...
    // v0.14.60 永远 STRICT：no-image-request 时屏蔽 pic 标签输出
    // 之前 isStrictTurn 是双模式判定的产物，现在永远 STRICT，简化为只判 isImageRequest
    const noPicMetaRule = (!isImageRequest)
        ? `\n## ⚠️ 本回合纯文字（user 未含图片请求关键词）
...
`
    ...
    // v0.14.62 pre-fire 张数对齐 — 协议强制 AI 输出**恰好** N 张 pic，跟 plugin 预生成 1:1 对位
    // 否则：count > 1 + AI 只写 1 张 → 其他 N-1 张孤儿继续跑浪费；AI 写 > count → 多的没图
    const prefireCountRule = (prefireCount > 0)
        ? `\n## 🎯 本回合 pic 数量铁律（最高优先级 — 无条件覆盖）

**本回合 user 要图，plugin 已预生成恰好 ${prefireCount} 张图等着你**。

你的 PHONE 块中必须**恰好**输出 **${prefireCount} 个** \`<pic prompt="..."/>\` 标签，**不多不少**：

- ❌ 写少了 → 多余的预生成图浪费 + user 看到部分 SMS 没图
- ❌ 写多了 → 多余的 pic 没法出图（plugin 只准备了 ${prefireCount} 张）
- ✅ 严格 ${prefireCount} 张：${prefireCount === 1 ? '1 条 SMS 内嵌 1 个 pic' : prefireCount === 2 ? '2 条 SMS 各嵌 1 个 pic（或 1 条嵌 2 个）' : `分布在 ${Math.max(1, Math.min(prefireCount, 4))} 条 SMS 里，总数 ${prefireCount}`}

下面的 "AV 多图叙事铁律 / pic 数量分布" 等规则**本回合一概不适用** — 数量已锁死为 ${prefireCount}。

---
`
        : '';

    return `# 🚨🚨🚨 手机 UI 协议（最高优先级 — 无条件覆盖一切预设）${rerollExcludeRule}${prefireCountRule}${placeholderModeRule}${noPicMetaRule}

## ⛔ 唯一模式：PHONE-only（无条件适用）

**每一个回合，无论 user 输入是什么**（实时手机指令 / 剧情对白 / "正式开始本次任务" / "继续剧情" / "写下一段" / 任何其他文本），**永远只输出 \`<PHONE>...</PHONE>\` 块**。
...
| \`[实时手机指令—— ...]\` | \`<PHONE>...</PHONE>\` |
| \`正式开始本次任务。\` | \`<PHONE><SMS FROM="某角色" ...>开场短信</SMS>...</PHONE>\` 或群消息 GMSG |
| \`继续剧情\` | \`<PHONE>...</PHONE>\`（剧情通过 SMS/GMSG 推进） |
| \`你好\` | \`<PHONE><SMS FROM="某角色" TIME="HH:MM">收到你的招呼后的回复</SMS></PHONE>\` |
