...
**⚠️ 构图视角铁律（v0.14.92 加） — 严禁默认 selfie**

selfie / looking at viewer / mirror selfie 这组 tag **只在以下场景**用：
- ① 角色主动用手机自拍发给 user（"我拍张照给你看"/"看看我今天穿啥"）
- ② user 明确要求自拍/镜头/对镜（"给我自拍"/"拍张照"）
- ③ 角色对着镜子自我展示（穿搭/化妆/挑衅 等）

**其他场景（占绝大多数 NSFW 性行为 + 偷拍 + 旁观 + 第三人称叙事）一律改用对应视角：**

| 场景 | 应用视角 / 构图 |
|---|---|
| 性爱 missionary 体位 | \`pov, from above, 1boy looking down, hetero, missionary\` |
| 性爱 cowgirl 体位 | \`pov, from below, cowgirl position, looking down at viewer, hetero\` |
| 性爱 doggystyle | \`from behind, doggystyle, ass focus, pov\` 或 \`from side, hetero\` |
| 性爱 mating press | \`pov, from above, mating press, full nelson, 1boy looking down\` |
| 偷窥 / 偷拍 | \`voyeur, hidden camera, candid photo, from hidden angle, unaware, not looking at camera\` (**严禁** selfie) |
| 旁观角度叙事 | \`third person view, from a distance, full body, looking away\` |
| 用户视角看角色 | \`pov, looking at viewer\`（**不加 selfie**） |
| 全景叙事场景 | \`wide shot, full body, scenic background\` |
| 物品/特写 | \`close up, object focus, no humans\` |

**❌ 反例**：
- 性行为场景写 \`1girl, selfie, looking at viewer, mating press\` → selfie 跟 mating press 矛盾，模型会画 1girl 自拍举手机 + 不像 mating press
- 偷拍剧情写 \`1girl, selfie, voyeur\` → 同样矛盾

**✅ 正例**：
- mating press 写 \`pov, from above, hetero, mating press, full nelson, 1boy looking down, on bed\`
- 偷拍写 \`voyeur, hidden camera, candid, from hidden angle, 1girl, undressing, unaware, bedroom\`
...
- 评论之间互相 @ / 接龙也可以（用 REPLY_TO 属性指向另一个评论者昵称）`;
    ...
    // v0.14.94 应用 v0.14.83 的 bookend + 去 pink-elephant 两个 lesson 到命令发帖路径：
    //   - bookend：user 指令在 OOC 头部+尾部各贴 1 次，防 AI 注意力被中间几千字协议吃掉
    //   - 删 "正式开始任务" 字面提及（AI 反向 fixate 这字串当指令）
    const userInstructionBlock = `‼️ user 这次在手机 UI 里点击的发帖指令：
=== user 指令 ===
${instruction}
=== 完 ===

⚠️ 三个等号之间是 user 让 ${targetName} 在 ${platform} 发帖的真实指令内容，必须读懂内容后按指令的语气和露骨程度兑现。
`;

    const userInstructionFooter = `

🔁 最后复读 — user 这次让 ${targetName} 在 ${platform} 发的内容是（必须直接兑现）：
=== user 指令 ===
${instruction}
=== 完 ===
`;
...
**【实时手机指令·最高优先级·覆盖格式/章节预设】**
本消息是 user 在手机 UI 里实时点击的命令，**不是**故事章节起点、不是预设触发短语、不是创作准则触发。
...
${platformExtra}${commentBlock}${userInstructionFooter}`;
}
...
export function buildGroupPostCommandOOC({
    ...
    // v0.14.94 bookend user 场景描述（与 buildPostCommandOOC 同思路）
    // 群聊生图协议有几千字，AI 容易丢失 user 场景重点
    const sceneBlock = scene ? `‼️ user 这次在群聊「${groupName}」点击的实时生图场景：
=== user 场景 ===
${scene}
=== 完 ===
...
` : '';
    ...
    const baseHeader = `${sceneBlock}**【实时群聊生图指令·最高优先级·覆盖格式/章节预设】**
