...
// <NPC_PROFILE NAME="X" KIND="real_origin_female|fictional_female|..." WORLDBOOK="Y">
//   姓名：...
//   年龄：...
//   身份：...
//   身体：...
...
//   称呼{{user}}：...
// </NPC_PROFILE>
// plugin 解析后存到 strangerAnchor.profile，升级为联系人时写入 contact.rawContent。
export function extractNpcProfiles(block) {
    ...
    while ((m = re.exec(block)) !== null) {
        ...
        // 从 profile 文本里抽 "外貌 booru：" 行做 core anchor
        let coreBooru = '';
        const booruMatch = profileText.match(/外貌\s*booru[：:]\s*([^\n]+)/i);
        if (booruMatch) coreBooru = booruMatch[1].trim();
        else {
            // fallback：找含 "1girl" / "1boy" 的英文行
            const fallback = profileText.split('\n').find(l => /\b1(?:girl|boy)\b/i.test(l));
            if (fallback) coreBooru = fallback.trim();
        }
        out.push({
            name,
            kind: (a.KIND || 'fictional_female').trim(),
...
`;
    ...
    // v0.14.40 严格图片触发铁律 — 仅当 user 含明确图片请求关键词时才允许嵌 <pic>
    // v0.14.41 同时强化"必须输出 PHONE 块"的铁律，防 AI 在 no-image 路径下脱缰写 prose
    const noPicMetaRule = (isStrictTurn && !isImageRequest)
        ? `\n## ⚠️⚠️⚠️ 本回合 STRICT 模式纯文字 PHONE 块（最高优先级，两条同时遵守）

**铁律 1：本回合严禁主动出图**
**本回合 user 消息未含图片请求关键词**（拍照/拍腿/拍奶/发图/发照片/发自拍/看看你/给我看/让我看/视频/直播/走光/穿搭/镜子/show me/selfie 等）。

- ❌ **严禁主动在 SMS / GMSG / 任何标签内嵌入** \`<pic prompt="..."/>\` 标签
- ❌ 严禁 AV 多图叙事 / 多张图序列 / STAGE 标签输出
- ✅ 即使 user 命令的是性行为（"操她" / "群战" / "大家轮流" / "调教她"）也按"文字描述"模式输出，**不嵌任何 pic 标签**

**铁律 2：必须输出 \`<PHONE>...</PHONE>\` 块，块外一字不写**

- ✅ 你的**整条**回复必须**完整包裹在** \`<PHONE>...</PHONE>\` 里，PHONE 块外**零字符**
- ✅ 文字内容必须用 \`<SMS FROM="X" TIME="HH:MM">文字</SMS>\` 或 \`<GMSG FROM="X" GROUP="X" TIME="HH:MM">文字</GMSG>\` 包裹
- ❌ **严禁**在 ST 主聊天框直接写 prose / 散文 / 章节正文（必须进 PHONE 块的 SMS/GMSG 标签里）
- ❌ **严禁**写 "好的，我来回复" / "她说道：" 等元叙述（直接用 SMS/GMSG 标签写角色说的话）
- ❌ **严禁** PHONE 块前后有任何 prose 引言/总结（块外严格 0 字符）

正确格式（仅文字 SMS，无 pic）：
\`\`\`
<PHONE>
<SMS FROM="清霜" TIME="23:15">嗯～夫君今晚想我了？</SMS>
<SMS FROM="清霜" TIME="23:15">人家在书房等你哦</SMS>
</PHONE>
\`\`\`

下面的协议里所有 \`<pic>\` 语法 / NSFW pic 维度 / pose 组合 / 镜头视角 等内容**仅供未来 user 明确要图时参考**，本回合**一概不许使用**。

---
`
        : '';

    return `# 🚨 手机 UI 协议（手机扩展已启用 — 双模式：实时指令 PHONE-only / 普通剧情 PHONE 可选）${noPicMetaRule}
...
## 引入新 NPC 时**必须**输出 NPC_PROFILE（结构化人设）

⚠️ 当 AI 引入**不在联系人列表 / 群成员档案 / 世界书条目**里的全新 NPC 时（如剧情中宫霄月给 user 推荐新警员"林雨桐"），**必须**在引入该 NPC 的 SMS/GMSG 之前输出一个 \`<NPC_PROFILE>\` 标签提供完整人设。

**强制要求**：
1. **符合世界观**：NPC 人设必须跟当前 worldbook lore + 角色卡情境一致（陆氏天下 = 东亚财阀都市 / 玉霄界 = 仙侠 / 街霸 = 武术 等）
2. **完整字段**：姓名、年龄、身份、身体（身高/体重/罩杯/体型）、外貌 booru、性格、口癖、与 {{user}} 关系、称呼 {{user}}、入场情境
3. **外貌 booru**：必须含英文 booru tag list（plugin 用它做角色 SD anchor）
4. **kind 属性**：
   - \`real_origin_female\` = 写实世界观女角色（如陆氏天下/锁情咒/公爵少爷）
   - \`fictional_female\` = 二次元/古风女角色（如玉霄界/苍宇寰界修仙）
   - \`fictional_male\` = 男角色

**NPC_PROFILE 格式**（写在 PHONE 块内，先于该 NPC 的 SMS/GMSG）：
\`\`\`
...
外貌 booru：1girl, brown hair, medium long hair, ponytail, brown eyes, mature female, tall, large breasts, athletic body, narrow waist, healthy skin, sharp eyes, police uniform, dark blue police shirt, mini skirt, black stockings, peaked cap, badge
...
Plugin 解析后会：
- 把外貌 booru 存到 chat-scoped strangerAnchor（**保证该 NPC 再次出现时视觉一致**）
- 整段人设存进 strangerAnchor.profile
- UI 在陌生人 tab 显示该 NPC，user 可一键"升级为正式联系人"（写入正式 contacts + sourceBook 自动锚定当前世界书）

**禁止**：
- ❌ 引入新 NPC 但不写 NPC_PROFILE（图会出，但 plugin 无法保存视觉锚 + 人设，下次再出现就变样）
- ❌ NPC_PROFILE 跟 worldbook 风格不符（古风世界观写出 "police officer / corporate exec" 是错的）
- ❌ NPC_PROFILE 名字跟已有联系人/群成员/世界书人物条目重名（让 user 觉得"她也升级了"困惑）
- ❌ 频繁引入新 NPC（一回合最多 1-2 个，剧情上 user 没明确要求就别滥引）
...
- 含动作动词的指令走系统提示"群聊 AV 多图叙事铁律"：主线 pic GMSG（含 @@STAGE:xxx@@）+ 反应 GMSG（无 STAGE 纯文字评论）；无动作动词的纯聊天保持 1-3 条文字 GMSG${imageHint}${countHint}${bodyPartHint}${continuityHint.replace('${targetName}', '群成员')}`;
    }
