...
**MOMENTS / GMSG 发帖图片额外要求**（SFW 帖子也要有场景）：
...
${currentModel === 'wai_anihentai'
    ? `**当前 model = wai_anihentai (anime)** —— 用 booru character anchor tag (高效，模型对动漫角色 prior 强)
格式：全小写 + 角色名 空格 + 来源括号
| 中文名 | booru tag |
|---|---|
...
不知道角色 booru 名字 → 退回类别 B/C。`
    : `**当前 model = asian_realism (写实)** —— 写实模型对动漫/真人 prior 弱，**不要**用 booru character anchor tag
