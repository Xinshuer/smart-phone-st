...
`
    ...
    // v0.14.53 双 Pass 加速模式 — 主 AI 用 <pic/> 占位符，pic prompt 由 plugin 后台单独生成
    // 关键澄清：本规则**只是替换 prompt 属性的内容**，pic 标签本身的所有使用规则（何时出图、几张图、SUBJECT、AV 多图分布等）全部照旧
    const placeholderModeRule = splitPicGen
        ? `\n## ⚡ 双 Pass 加速模式（本插件已开启 — 最高优先级）\n\n### 🚨 唯一区别：只省略 prompt 属性的内容，pic 标签照常输出！\n\n**本规则只是把 \`<pic prompt="...long booru string..."/>\` 简化成 \`<pic/>\`，所有其他 pic 规则一字不变。**\n\n#### ✅ MUST：所有该出图的地方都要正常输出 \`<pic/>\` 标签\n\n- user 让发自拍 → SMS 内**必须**嵌 \`<pic/>\`\n- user 让发腿照/胸照/身体局部 → SMS 内**必须**嵌 \`<pic/>\`\n- 朋友圈 / MOMENTS / 小红书 / 论坛配图 → **必须**嵌 \`<pic/>\`\n- AV 多镜头叙事（user 命令性爱）→ 按下面铁律输出**多个** \`<pic/>\`（数量/位置/分布完全照旧）\n- A 发 B 的照片 → \`<pic SUBJECT="B"/>\`（SUBJECT 属性照常带）\n- 合影 → \`<pic SUBJECTS="A,B"/>\`\n\n#### ❌ 唯一禁止：不要写 \`prompt="..."\` 属性\n\n- ✅ 正确：\`<SMS FROM="清霜" TIME="14:32">夫君你看～ <pic/></SMS>\`\n- ✅ 正确：\`<SMS FROM="清霜" TIME="14:32">人家穿了新衣服 <pic SUBJECT="清霜"/></SMS>\`\n- ❌ 错误（写了 prompt 属性）：\`<SMS><pic prompt="1girl, selfie, ..."/></SMS>\`\n- ❌ 错误（该出图却没出）：\`<SMS FROM="清霜" TIME="14:32">夫君你看～</SMS>\` （user 让发自拍但没 \`<pic/>\`）\n- ❌ 错误（用文字占位）：\`<SMS>[图片]</SMS>\` / \`<SMS>(发了张自拍)</SMS>\`\n\n#### 📝 原理（让你理解为啥这么干）\n\nplugin 后台会用另一个 quiet AI 单独为每个 \`<pic/>\` 生成 booru prompt（基于这条 SMS 文本 + 角色视觉档案 + user 原话），所以**你不用操心 prompt 内容**，但**pic 标签该出现的位置必须出现**，否则后台 AI 没东西可以填，user 就看不到图。\n\n下面协议里所有关于"pic prompt 必须含 XX tag"的规则（如"拍腿要含 legs focus"），本回合**只是这些 tag 不用你写**——但**该出 pic 标签的场景一个都不能漏**，pic 标签的位置/数量/SUBJECT/AV 多镜头分布等结构性规则**全部照旧执行**。\n\n---\n`
        : '';

    return `# 🚨 手机 UI 协议（手机扩展已启用 — 双模式：实时指令 PHONE-only / 普通剧情 PHONE 可选）${rerollExcludeRule}${placeholderModeRule}${noPicMetaRule}
