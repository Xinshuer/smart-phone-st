...
`
    ...
    // v0.14.56 双 Pass 模式下，protocol 整段后处理：所有 <pic prompt="..."/> 示例改写成 <pic/>，
    // 防止 AI 看到完整 prompt 示例后跟着写。包装在 IIFE 里返回，保持原有 return 语义。
    const _rawProto = `# 🚨 手机 UI 协议（手机扩展已启用 — 双模式：实时指令 PHONE-only / 普通剧情 PHONE 可选）${rerollExcludeRule}${placeholderModeRule}${noPicMetaRule}
...
${includeAVSections ? (buildAVNarrativeSection() + buildGroupAVNarrativeSection()) : ''}`;
    ...
    return splitPicGen ? _stripPromptsForPlaceholderMode(_rawProto) : _rawProto;
}

// v0.14.56 双 Pass 模式下，整个 protocol 后处理：所有 <pic prompt="..."/> 示例改写成 <pic/>，
// 防 AI 看到几十个完整 prompt 例子被诱导跟着写完整 prompt → placeholderModeRule 失效，
// Pass 2 检测不到占位符，Pre-fire ComfyUI 不启动，整条并行链失败。结构铁律（多 SMS 线性叙事、
// SUBJECT 属性、AV 多镜头数量分布）保持不变，只是 prompt 内容从示例和指令里剥掉。
function _stripPromptsForPlaceholderMode(text) {
    if (!text) return '';
    // 1) 完整闭合的 <pic prompt="..."/> → <pic/>（保留 SUBJECT/SUBJECTS 等其他属性）
    let out = text.replace(/<pic\s+prompt="[^"]*"([^>]*)\/>/gi, (_m, rest) => `<pic${rest || ''}/>`);
    // 2) "pic prompt 以 @@STAGE:xxx@@ 标签开头" → 改写
    out = out.replace(/pic\s*prompt\s*以\s*`@@STAGE:[^`]*`\s*标签开头/gi, 'pic 占位符空 `<pic/>` 即可（plugin 后台生成 prompt 时会注入 STAGE）');
    return out;
}
