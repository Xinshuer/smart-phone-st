...
export function renderImagesGrid(images, options = {}) {
    ...
    let { hint = '', context = '', source = '', allowReroll = false, subjects = '', subject = '' } = options;
    ...
    const cells = images.slice(0, 9).map((entry) => {
        if (isImageUrl(entry)) {
        ...
        if (isPicTag(entry)) {
            const rerollBtn = allowReroll
                ...
                : '';
            return `<div class="phone-img-cell">
                <div class="phone-image-slot" data-pic="${escapeHtml(entry)}" ${hintAttr} ${ctxAttr}${srcAttr}${subjectsAttr}>📷 生成中…</div>
                ${rerollBtn}
            </div>`;
        }
        // Unknown — skip
