...
export function renderMessageList(chatId) {
    ...
    // v0.14.1 聊天 tab 语义改为「最近会话」(类微信)：
    //   - 仅显示有消息的单聊 (msgs.length > 0)
    //   - 所有未删除群聊（创建群本身就是显式聊天意图）
    //   - 无消息联系人 → 切到「联系人」tab 点 💬 开始聊天
    const activeContacts = s.contacts.filter(c => (cs.threads[c.name] || []).length > 0);
    const contactItems = activeContacts.map(c => ({
        kind: 'contact', name: c.name, msgs: cs.threads[c.name] || [],
    }));
    ...
    if (contactItems.length === 0 && groupItems.length === 0) {
        ...
        const hint = totalContacts > 0
            ? `已有 ${totalContacts} 个联系人，去「👤 联系人」tab 选一个开始聊天`
            : '从「设置 → 世界书条目」导入联系人';
        return `
    ...
    // 联系人没参考图时用渐变首字头像（mochi 风格）
    const renderContactAvatar = (name, refImg) => {
        if (refImg) return `<img class="phone-avatar phone-avatar-ref" src="${escapeHtml(refImg)}" alt="${escapeHtml(name.slice(0, 1))}" onerror="this.outerHTML='${renderGradientInitial(name)}'">`;
        return renderGradientInitial(name);
    };

    // 无消息的联系人提示条（让用户知道还有更多联系人可聊）
    const inactiveContactsCount = s.contacts.length - activeContacts.length;
    const moreHint = inactiveContactsCount > 0
        ? `<div class="phone-list-more-hint">还有 ${inactiveContactsCount} 个联系人没聊过 → 切到「👤 联系人」tab 开始</div>`
        : '';
