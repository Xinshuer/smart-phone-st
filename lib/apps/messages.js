...
export function renderMessageList(chatId) {
    ...
    // Only show contacts that are currently active (filtered by sourceBook in v0.10.4+).
    // Orphan threads from previous worldbooks are hidden but their data is preserved in
    // cs.threads — re-importing the contact (e.g. switching back to that worldbook) will
    // restore the thread because lookups are keyed by contact name.
    const seen = new Map();
