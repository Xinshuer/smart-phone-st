...
// → 返回"🆕 林雨桐 [一键升级]" chip HTML
function renderNewNpcChip(m) {
    if (!m.subject) return '';
    // 检查 SUBJECT 是不是 stranger (待升级状态)
    // 不能直接 import State（循环 import），用 window.smartPhone API
    ...
    if (isContact) return ''; // 已经是联系人，不需要 🆕 标记
    const sa = sp.getStrangerAnchor?.(ctxId, m.subject);
    if (!sa) return ''; // 既不是联系人也不是 stranger（应该有 toast 警告了）
    return `<button class="phone-new-npc-chip" data-npc-name="${escapeHtml(m.subject)}" title="点击升级为正式联系人">🆕 新认识 ${escapeHtml(m.subject)} · 一键保存</button>`;
}
    ...
    switch (m.type) {
        ...
        default:
            ...
            if (m.pic) {
                ...
                return `${escapeHtml(m.content)}${renderMsgPic(m.pic, m.subject || '')}`;
            }
