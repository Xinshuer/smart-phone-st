...
function renderMomentCard(p, contact, userName) {
    ...
    const picHtml = p.pic
        ? `<div class="phone-image-slot" data-pic="${escapeHtml(p.pic)}" data-hint="${escapeHtml(p.from || '')}" data-context="${escapeHtml(p.content || '')}" style="max-width:180px;margin-top:6px;">📷 生成中…</div>`
        : '';
