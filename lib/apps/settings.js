...
const collapsedSections = new Set(['worldbook', 'storage']);
        ...
        <div class="phone-settings">
            ...
            <section class="phone-settings-section">
                ...
                <select id="phone-model-select" class="phone-select">
                    <optgroup label="动漫">
                        <option value="wai_anihentai" ${s.imageGen.currentModel === 'wai_anihentai' ? 'selected' : ''}>waiANIHENTAIPONYXL v60 (动漫·默认)</option>
                        <option value="unholy_desire" ${s.imageGen.currentModel === 'unholy_desire' ? 'selected' : ''}>Unholy Desire Mix Sinister v8 (动漫·快速 16 步)</option>
                        <option value="diving_illustrious" ${s.imageGen.currentModel === 'diving_illustrious' ? 'selected' : ''}>Diving Illustrious Flat v7 (动漫·flat 风格)</option>
                        <option value="nova_asian_il" ${s.imageGen.currentModel === 'nova_asian_il' ? 'selected' : ''}>Nova Asian XL IL v7 (动漫·亚洲脸特化)</option>
                        <option value="nova_orange_xl" ${s.imageGen.currentModel === 'nova_orange_xl' ? 'selected' : ''}>Nova Orange XL Rex v1 (动漫·美学高清)</option>
                    </optgroup>
                    <optgroup label="写实">
                        <option value="asian_realism" ${s.imageGen.currentModel === 'asian_realism' ? 'selected' : ''}>illustAsianCoser v3 (亚洲写实·cosplay 棚拍)</option>
                        <option value="lustify_v8" ${s.imageGen.currentModel === 'lustify_v8' ? 'selected' : ''}>LUSTIFY! v8 Apex (欧美写实 NSFW·原生 1536px)</option>
                    </optgroup>
                </select>
                <label class="phone-settings-row">
                    💻 ComfyUI 地址（电脑）
                ...
                <label class="phone-settings-row" style="margin-top:8px">
                    <input type="checkbox" id="phone-split-pic-gen" ${s.imageGen.splitPicGen ? 'checked' : ''}>
                    ⚡ 双 Pass 出图加速（实验）
                    <span class="phone-settings-hint" style="margin-top:4px">主 AI 只出 SMS 文字+占位符 → 后台 quiet AI 单独生成 booru prompt → ComfyUI 出图。<br>预期：纯聊天 65% 加速，含图聊天 40-55% 加速。</span>
                </label>
                </div>
            </section>
            ...
            <section class="phone-settings-section">
                <div class="phone-settings-header">
                    <h3>🧹 存储管理</h3>
                    ${sectionToggle('storage')}
                </div>
                ${sectionBodyOpen('storage')}
                <p class="phone-settings-hint">删除酒馆角色卡后，对应 chat 数据不会自动清理。点下方按钮查看所有 chat state 并选择性清理。</p>
                <button id="phone-cleanup-orphan-btn" class="phone-btn">🧹 查看并清理 chat state</button>

                <!-- v0.14.86 移动端友好的图片 URL 重置（替代 F12 控制台脚本） -->
                <p class="phone-settings-hint" style="margin-top:16px;">删了 ComfyUI output 文件夹后，plugin 里缓存的 URL 全失效（坏图）。点这俩按钮一键重置：</p>
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    <button id="phone-reset-pic-urls-btn" class="phone-btn" title="清掉所有 chat 的 picUrls 缓存 + feed post.images（resolvedFromPic 自动写回的）。保留人物头像 ✨ 图。">
                        🗑️ 清图片缓存（保留 ✨）
                    </button>
                    <button id="phone-reset-all-images-btn" class="phone-btn" title="核弹：清掉 picUrls + post.images + 所有联系人 anchor.referenceImage（✨ 图也清）。点了之后头像变首字母，需要手动重生 ✨。" style="background:#e88;">
                        🔥 全清（含 ✨ 图）
                    </button>
                </div>
                </div>
...
export function bindSettingsHandlers(root, {
    ...
    onEditContactSourceBook, onCleanupOrphanChats,
    onResetPicUrls, onResetAllImages,  // v0.14.86 移动端友好图片 URL 重置
}) {
    ...
    root.querySelector('#phone-cleanup-orphan-btn')?.addEventListener('click', () => onCleanupOrphanChats && onCleanupOrphanChats());
    // v0.14.86 图片 URL 重置（移动端友好）
    root.querySelector('#phone-reset-pic-urls-btn')?.addEventListener('click', () => onResetPicUrls && onResetPicUrls());
    root.querySelector('#phone-reset-all-images-btn')?.addEventListener('click', () => onResetAllImages && onResetAllImages());
    ...
    // v0.14.49 双 Pass 出图加速 toggle
    // v0.14.52 fix：原代码用了 loadState()/saveState() 但 import 的是 load/save，
    // TypeError 在 event handler 里静默失败，导致 toggle 永远存不下来
    const splitPicGenCheckbox = root.querySelector('#phone-split-pic-gen');
    if (splitPicGenCheckbox) splitPicGenCheckbox.addEventListener('change', () => {
        const s = load();
        if (!s.imageGen) s.imageGen = {};
        s.imageGen.splitPicGen = splitPicGenCheckbox.checked;
        save();
        console.log(`[smart-phone v0.14.52] splitPicGen toggle → ${splitPicGenCheckbox.checked}`);
    });
