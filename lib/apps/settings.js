...
// 世界书条目 and 内容生成世界观 default to collapsed (can be long).
const collapsedSections = new Set(['worldbook', 'worldContext']);
...
export async function renderSettings() {
    ...
    return `
        <div class="phone-settings">
            ...
            <section class="phone-settings-section">
                <div class="phone-settings-header">
                    <h3>📖 内容生成世界观（${worldCtx.length}）</h3>
                    ${sectionToggle('worldContext')}
                </div>
                ${sectionBodyOpen('worldContext')}
                <p class="phone-settings-hint">勾选后，AI 在生成小红书/论坛帖子时会参照这些世界书条目的设定，让内容贴合世界观。</p>
                <div class="phone-wb-list">
                    ${lores.length === 0
                        ? '<div class="phone-empty-inline">无世界观条目（先刷新世界书）</div>'
                        : lores.map((e) => {
                            const isIn = worldCtx.some((w) => w.uid === e.uid && w.bookName === e.bookName);
                            return `
                                <div class="phone-wb-row" data-uid="${e.uid}" data-book="${escapeHtml(e.bookName)}">
                                    ...
                                    <button class="phone-btn phone-toggle-wc ${isIn ? 'phone-wc-active' : ''}"
...
export function bindSettingsHandlers(root, { onImportContact, onGenRef, onLockRef, onRemoveContact, onModelChange, onComfyuiUrlChange, onToggleLore, onToggleWorldContext, onRefresh, onApiSave, onApiTest, onApiTriggerNow, onComfyuiTest, onFetchModels, onPromptEdit, onGenAppearance }) {
    // Section collapse toggles
    root.querySelectorAll('.phone-section-toggle').forEach((btn) => {
