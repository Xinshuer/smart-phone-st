...
function renderDetail(chatId, postId) {
    ...
    return `
        <div class="xhs-detail">
            ...
            <div class="xhs-detail-img phone-image-slot" data-pic="${escapeHtml(picTag)}" data-hint="${escapeHtml(p.user || '')}" data-context="${escapeHtml((p.title || '') + ' ' + (p.body || ''))}"></div>
            <div class="xhs-detail-img-actions">
                <button class="xhs-reroll-pic-btn" data-pic="${escapeHtml(picTag)}" title="重新生成图片">🔄 重新生成</button>
            </div>
...
export function bindXHSHandlers(root, { onCompose, onRefresh, onClear, onOpenPost, onBackToFeed, onSubmit, onTagSelect, onRerollPic }) {
    ...
    root.querySelector('.xhs-reroll-pic-btn')?.addEventListener('click', (e) => {
