...
import { escapeHtml, renderImagesGrid, readPostImages, isImageUrl, isPicTag } from '../util.js';
...
// 此前 .xhs-card-img 留空 → 所有卡片永远只是骨架 shimmer 占位，从不显示真实图。
// 设计：直接放 phone-image-slot（不走 phone-img-grid 包装）—— 现有 CSS
// `.xhs-card-img img.phone-pic { width:100%!important; height:100%!important; object-fit:cover }`
// 直接命中，slot 由 triggerPicSlots 异步生成 ComfyUI 图后替换成 <img>。
function renderCardImg(p) {
    const images = readPostImages(p);
    if (!images.length) return ''; // CSS :not(:has(img)) shimmer 兜底
    const first = images[0];
    if (isImageUrl(first)) {
        return `<img class="phone-pic" src="${escapeHtml(first)}" loading="lazy">`;
    }
    if (isPicTag(first)) {
        const hint = escapeHtml(p.user || '');
        const context = escapeHtml((p.title || '') + ' ' + (p.body || ''));
        const source = p.from === 'user' ? 'xhs_user' : (p.commandedByUser ? 'xhs_command' : 'xhs');
        return `<div class="phone-image-slot" data-pic="${escapeHtml(first)}" data-hint="${hint}" data-context="${context}" data-source="${source}">📷 生成中…</div>`;
    }
    return '';
}
