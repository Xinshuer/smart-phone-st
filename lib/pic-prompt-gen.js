...
// v0.14.50 ⭐⭐ 真并行 Pass 2 — 用独立 phone-api fetch 通道，跟 ST 主流不冲突
// generateQuietPrompt 共享 abortController 必须串行，callPhoneApi 走独立 HTTP fetch
// 可以跟 ST 主流并发跑 → 实现"两个 agent 同时工作"
//
// 用法：onPromptReady 阶段订阅 STREAM_TOKEN_RECEIVED，每检测到完整 <SMS> 单元含 <pic/> →
// 立即 fire 这个 Promise 跑 Pass 2，存到 pendingPromises Map。onMessageReceived 时 await
// 所有 pending Promises 拿到 prompts，填回 m.pic。
export async function generatePicPromptViaPhoneApi(ctx) {
    const {
        targetName = '',
        smsContent = '',
        userText = '',
    ...
    // 检查 phone-api 是否配置 — 无 url/key 则返 null，让 caller fallback 到 generateQuietPrompt
    const s = loadState();
    const cfg = s.api || {};
    if (!cfg.url || !cfg.key) {
        console.warn('[smart-phone v0.14.50 Pass 2 parallel] phone-api 未配置（缺 url/key），无法并行');
        return null;
    }
    ...
    // userPrompt 短而明确，避免被预设/JailbreakPrompt 干扰
    const userInput = `角色：${targetName}\nSMS：${smsContent}\nuser 原话：${userText}\n\n输出 1 行英文 booru prompt：`;

    try {
        // useFallback=false — 不走 ST generateRaw 回退（那会共享 abortController 串行化）
        ...
        const extracted = extractBooruPrompt(raw);
        if (!looksLikeBooruPrompt(extracted)) {
            console.warn(`[smart-phone v0.14.50 Pass 2 parallel] AI 输出不合规：${raw?.slice(0, 200)}`);
            return FALLBACK_PROMPT;
        }
        return extracted;
    } catch (err) {
        console.error('[smart-phone v0.14.50 Pass 2 parallel] failed:', err);
        return null;
    }
}
