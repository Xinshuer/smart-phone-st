...
export async function prefirePics({
    ...
    for (const prompt of prompts) {
        ...
        const p = smartImageGen.generateFromPicTag(tempPicTag, { contacts, hint })
            .catch((err) => {
                console.warn('[smart-phone v0.14.59 pre-fire] ComfyUI fire 失败:', err);
                return null;
            });
        urlPromises.push(p);
        console.log(`[smart-phone v0.14.59] ⚡⚡⚡ pre-fire ComfyUI (target=${target}, prompt=${prompt.slice(0, 60)}…)`);
    }
    return urlPromises;
}
